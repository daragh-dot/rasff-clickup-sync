...
RASFF_ITEMS_PER_PAGE = 99  # RASFF API rejects values >= 100 ("must be less than 100")
...
def fetch_notifications(lookback_days: int) -> list[Notification]:
    ...
    all_records: list = []
    page_number = 1
    max_pages = 20  # safety cap - 20 * 99 ~= 1,980 notifications, far more than any realistic window
    while page_number <= max_pages:
        payload["parameters"]["pageNumber"] = page_number
        resp = session.post(RASFF_EXPORT_API_URL, json=payload, timeout=60)
        if resp.status_code != 200:
            raise RuntimeError(
                f"RASFF API returned HTTP {resp.status_code} on page {page_number}. "
        ...
        try:
            ...
            raise RuntimeError(
                f"RASFF API did not return valid JSON on page {page_number} "
                f"(endpoint may have changed): {e}\n"
        ...
        if len(records) < RASFF_ITEMS_PER_PAGE:
            break  # last page reached
        page_number += 1
    else:
        print(f"WARNING: hit the {max_pages}-page safety cap - there may be more "
              f"notifications than were fetched. Consider narrowing LOOKBACK_DAYS.")

    return [Notification.from_api_record(rec) for rec in all_records]
